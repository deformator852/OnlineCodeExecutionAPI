...
from compiler.languages.types import ExecutorResult, ExecutorResultMetaData, AnalyzerResult
import time
...
class PythonExecutor(Executor):
    def execute(self, code: str) -> ExecutorResult:
        ...
        if not analyzer_result.status:
            meta_data = ExecutorResultMetaData(execution_time=0.0, output_result=analyzer_result.result)
            return ExecutorResult(False, meta_data)
        start_time = time.time()
        try:
            execution_result = subprocess.run(["python3", "-c", code], capture_output=True, text=True, timeout=7)
        except subprocess.TimeoutExpired:
            executor_result_meta_data = ExecutorResultMetaData(execution_time=7.0,
                                                               output_result="Your code executing too long.")
            return ExecutorResult(execution_status=False, meta_data=executor_result_meta_data)
        end_time = time.time()
